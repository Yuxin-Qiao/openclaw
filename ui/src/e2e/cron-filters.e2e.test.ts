...
describeControlUiE2e("Control UI cron mocked Gateway E2E", () => {
  ...
  it("localizes history filters and selects them using only native keyboard controls", async () => {
    ...
    try {
      ...
      await page.keyboard.press("Enter");
      await page
        .getByRole("button", { name: "Status OK, Fehler und Übersprungen", exact: true })
        .waitFor();

      await page.keyboard.press("Escape");
      await page.keyboard.press("Tab");
      await expect
        .poll(() => deliveryTrigger.evaluate((element) => element === document.activeElement))
        .toBe(true);
      await page.keyboard.press("Enter");
      ...
      await page
        .getByRole("button", {
          name: "Zustellung Zugestellt, Nicht zugestellt und Unbekannt",
          exact: true,
        })
        .waitFor();
